...
def model():
	...
	model = k.models.Sequential()
	model.add(k.layers.Conv2D(16, (2, 2), activation='relu', padding='same',
	                 input_shape = (c.imgWidth, c.imgHeight, c.imgChanel)))
	
	model.add(k.layers.Conv2D(32, (2, 2), activation='relu', padding='same'))
	model.add(k.layers.MaxPool2D(strides=(2,1)))
	# model.add(k.layers.Dropout(0.25))

	model.add(k.layers.Conv2D(64, (2, 2), activation='relu', padding='same'))
	model.add(k.layers.MaxPool2D(strides=(2,1)))
	...
	# model.add(k.layers.Dense(6000, activation='tanh'))
	# model.add(k.layers.Dropout(0.2))
	# model.add(k.layers.Dense(1024, activation='relu'))
	# model.add(k.layers.Dropout(0.2))
	# model.add(k.layers.Dense(100, activation='relu'))
	# model.add(k.layers.Dropout(0.2))
	model.add(k.layers.Dense(c.num_classes, activation='softmax'))
	# # model.add(k.layers.BatchNormalization())
